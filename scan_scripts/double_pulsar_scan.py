...
def print_status(ip, message):
    ...
    with print_lock:
        print("[*] [%s] %s".format(ip, message))

def check_ip(ip):
    global negotiate_protocol_request, session_setup_request, tree_connect_request, trans2_session_setup, timeout, verbose
    timeout = TIMEOUT
    verbose = True
    uninstall = False  # Set to True if you want to uninstall DOUBLEPULSAR
    user_id = "\x00\x00"  # Default user ID
    ...
    try:
        ...
        print_status(ip, "Failed to send negotiate protocol request")
    ...
    try:
        ...
        print_status(ip, "Failed to send session setup request")
    ...
    tree_id= session_setup_response[28:30]
    ...
    if final_response[34] == "\x51":
        ...
        with print_lock:
            print("[+] [%s] DOUBLEPULSAR SMB IMPLANT DETECTED!!! Arch: %s, XOR Key: %s".format(ip, arch, hex(key)))
        ...
        if uninstall:
            ...
            if uninstall_response[34] == "\x52":
                with print_lock:
                    print("[+] [%s] DOUBLEPULSAR uninstall successful".format(ip))

    else:
        with print_lock:
            print("[-] [%s] No presence of DOUBLEPULSAR SMB implant".format(ip))
